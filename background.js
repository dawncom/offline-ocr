const MODEL_FILE = "ppocr_rec.onnx";
const MODEL_CONFIG_FILE = "inference.yml";
const DEFAULT_MODEL_NAME = "PP-OCRv6 tiny rec";
...
const REC_MIN_WIDTH = 160;
const REC_MAX_WIDTH = 1600;
const REC_HEIGHT = 48;
const MIN_SELECTION_SIZE = 8;
const MAX_LINE_REGIONS = 16;

let recSession = null;
let ortRuntime = null;
let recInputName = "x";
let recOutputName = null;
let chars = [];
let initError = null;
let initPromise = initOCR();
let warnedClassMismatch = false;
let activeModelInfo = {
  source: "bundled",
  ...
  characters: 0,
  input: recInputName,
  output: recOutputName
};

function hideUMDGlobals() {
  const root = typeof window !== "undefined" ? window : globalThis;
  const hadModule = Object.prototype.hasOwnProperty.call(root, "module");
  const hadExports = Object.prototype.hasOwnProperty.call(root, "exports");
  const oldModule = root.module;
  const oldExports = root.exports;

  try {
    root.module = undefined;
    root.exports = undefined;
  } catch (error) {
    // Some globals may be non-writable. ORT still runs in the extension worker.
  }

  return () => {
    try {
      if (hadModule) root.module = oldModule;
      else delete root.module;
      if (hadExports) root.exports = oldExports;
...
async function initOCR() {
  let restoreUMDGlobals = null;
  try {
    restoreUMDGlobals = hideUMDGlobals();
    // ONNX Runtime Web is inlined below.
/*!
...
// end ort.min.js
    if (restoreUMDGlobals) {
      restoreUMDGlobals();
      restoreUMDGlobals = null;
    }

    ortRuntime = ort;
    ortRuntime.env.wasm.wasmPaths = chrome.runtime.getURL("");
    ortRuntime.env.wasm.simd = true;
    ortRuntime.env.wasm.proxy = false;
    ortRuntime.env.wasm.numThreads = 1;

    const modelAsset = await loadActiveModelAsset();
    const modelBuffer = modelAsset.modelBuffer;
    const modelConfig = modelAsset.modelConfig;
    chars = extractCharactersFromOnnx(modelBuffer);
    if (chars.length === 0 && modelConfig) {
      chars = extractCharactersFromInferenceYaml(modelConfig);
    }
    if (chars.length === 0) {
      throw new Error("OCR character set not found in ONNX metadata or inference.yml");
    }

    recSession = await ortRuntime.InferenceSession.create(modelBuffer, {
      executionProviders: ["wasm"],
      graphOptimizationLevel: "all"
    });
    recInputName = recSession.inputNames && recSession.inputNames[0] ? recSession.inputNames[0] : "x";
    recOutputName = recSession.outputNames && recSession.outputNames[0] ? recSession.outputNames[0] : null;
    activeModelInfo = {
      ...modelAsset.info,
      chars: chars.length,
    ...
    console.info("OCR model ready", {
      ...activeModelInfo,
      firstCharCodes: chars.slice(0, 12).map((char) => char.codePointAt(0))
    });
  } catch (error) {
    if (restoreUMDGlobals) restoreUMDGlobals();
    initError = error;
...
async function ensureOCRReady() {
  ...
  if (initError) {
    throw new Error("OCR init failed: " + (initError.message || String(initError)));
  }
  if (!recSession) {
    throw new Error("OCR engine is not ready yet");
  }
}

async function reloadOCRModel() {
  await releaseOCRSession();
  initError = null;
  warnedClassMismatch = false;
  initPromise = initOCR();
  await initPromise;
  if (initError) throw initError;
  return getOCRStatusSnapshot();
}

async function releaseOCRSession() {
  const oldSession = recSession;
  recSession = null;
  recInputName = "x";
  recOutputName = null;
  chars = [];
...
function getOCRStatusSnapshot() {
  return {
    ...activeModelInfo,
    ready: Boolean(recSession && !initError),
    error: initError ? initError.message || String(initError) : "",
    characters: chars.length || activeModelInfo.characters || 0,
    input: recInputName,
    output: recOutputName
  };
}

async function loadActiveModelAsset() {
  try {
    const custom = await loadCustomModelAsset();
    if (custom) return custom;
  } catch (error) {
    console.warn("Custom OCR model storage is unavailable; using bundled model", error);
  }
  return loadBundledModelAsset();
}

async function loadBundledModelAsset() {
  const modelUrl = chrome.runtime.getURL(MODEL_FILE);
  const response = await fetch(modelUrl);
  if (!response.ok) {
    throw new Error("Failed to load bundled OCR model: HTTP " + response.status);
  }

  const modelBuffer = await response.arrayBuffer();
  const modelConfig = await fetchOptionalText(MODEL_CONFIG_FILE);
  return {
    modelBuffer,
    modelConfig,
    info: {
      ...
      configBytes: byteLengthOfText(modelConfig),
      savedAt: ""
    }
  };
}

async function loadCustomModelAsset() {
  const db = await openModelDb();
  const modelBuffer = await modelDbGet(db, MODEL_KEY_ONNX);
  if (!modelBuffer) return null;

  const modelConfig = await modelDbGet(db, MODEL_KEY_CONFIG) || "";
  const meta = await modelDbGet(db, MODEL_KEY_META) || {};
  return {
    modelBuffer,
    modelConfig,
    info: {
      source: "custom",
      name: meta.name || meta.modelFileName || "Custom OCR rec model",
      modelBytes: modelBuffer.byteLength || meta.modelBytes || 0,
      configBytes: byteLengthOfText(modelConfig) || meta.configBytes || 0,
...
chrome.commands.onCommand.addListener(async (command) => {
  ...
  try {
    ...
    console.warn("Selection UI injection failed", error);
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "reloadModel") {
    reloadOCRModel()
      .then((status) => sendResponse({ ok: true, status }))
      .catch((error) => sendResponse({ ok: false, error: error.message || String(error), status: getOCRStatusSnapshot() }));
    return true;
  }
...
async function doOCR(selection, sender) {
  await ensureOCRReady();

  const rect = normalizeSelection(selection);
  const dataUrl = await chrome.tabs.captureVisibleTab(sender && sender.tab ? sender.tab.windowId : null, {
    format: "png"
  });
  const imgData = await cropVisibleSelection(dataUrl, rect);
  const regions = prepareRecognitionRegions(imgData);
  const lines = [];

  for (const region of regions) {
    const text = await recognizeRegion(region);
  ...
  return {
    ...
    modelSource: activeModelInfo.source || "bundled"
  };
}

function getActiveModelDisplayName() {
  return activeModelInfo && activeModelInfo.name ? activeModelInfo.name : DEFAULT_MODEL_NAME;
}

async function recognizeRegion(imgData) {
  const tensor = preprocess(imgData, REC_MAX_WIDTH, REC_HEIGHT);
  const feeds = {};
  feeds[recInputName] = tensor;

  const outputMap = await recSession.run(feeds);
  const output = recOutputName && outputMap[recOutputName] ? outputMap[recOutputName] : outputMap[Object.keys(outputMap)[0]];
  const text = fixSpaces(ctcDecode(output)).trim();
  return text;
}

function normalizeSelection(selection) {
  const x = Math.max(0, Math.round(Number(selection.x) || 0));
  const y = Math.max(0, Math.round(Number(selection.y) || 0));
  const w = Math.round(Number(selection.w) || 0);
  const h = Math.round(Number(selection.h) || 0);

  if (w < MIN_SELECTION_SIZE || h < MIN_SELECTION_SIZE) {
    throw new Error("Selection is too small; use at least 8px x 8px");
  }
  ...
  return {
    ...
    w,
    h,
    viewportWidth: Math.max(1, Number(selection.viewportWidth) || 1),
    viewportHeight: Math.max(1, Number(selection.viewportHeight) || 1)
  };
}

async function cropVisibleSelection(dataUrl, selection) {
  const blob = await fetch(dataUrl).then((response) => response.blob());
  const bitmap = await createImageBitmap(blob);
  const scaleX = bitmap.width / selection.viewportWidth;
  const scaleY = bitmap.height / selection.viewportHeight;
  const cropX = clamp(Math.round(selection.x * scaleX), 0, bitmap.width - 1);
  const cropY = clamp(Math.round(selection.y * scaleY), 0, bitmap.height - 1);
  const cropRight = clamp(Math.round((selection.x + selection.w) * scaleX), cropX + 1, bitmap.width);
  const cropBottom = clamp(Math.round((selection.y + selection.h) * scaleY), cropY + 1, bitmap.height);
...
function prepareRecognitionRegions(imgData) {
  const trimmed = trimImageDataMargins(imgData);
  const lines = splitTextLines(trimmed);
  if (lines.length <= 1) return [trimmed];
  return lines.slice(0, MAX_LINE_REGIONS).map(trimImageDataMargins);
}

function trimImageDataMargins(imgData) {
  const width = imgData.width;
  const height = imgData.height;
  if (width < 12 || height < 12) return imgData;
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (colorDistance(data, offset, bg) < 50) continue;
      if (x < minX) minX = x;
      if (x > maxX) maxX = x;
      if (y < minY) minY = y;
      if (y > maxY) maxY = y;
  ...
  const padX = Math.min(10, Math.max(3, Math.round(width * 0.01)));
  const padY = Math.min(8, Math.max(2, Math.round(height * 0.04)));
  minX = clamp(minX - padX, 0, width - 1);
  maxX = clamp(maxX + padX, minX, width - 1);
  minY = clamp(minY - padY, 0, height - 1);
  maxY = clamp(maxY + padY, minY, height - 1);

  const cropW = maxX - minX + 1;
  const cropH = maxY - minY + 1;
  if (cropW >= width - 2 && cropH >= height - 2) return imgData;
  if (cropW < MIN_SELECTION_SIZE || cropH < MIN_SELECTION_SIZE) return imgData;
  return cropImageData(imgData, minX, minY, cropW, cropH);
}

function splitTextLines(imgData) {
  const width = imgData.width;
  const height = imgData.height;
  if (height < 24) return [imgData];

  const rowInfo = buildRowActivityScores(imgData);
  const rowScores = rowInfo.scores;
  const maxScore = rowInfo.maxScore;

  if (maxScore < 3) return [imgData];

  const activeThreshold = Math.max(2, Math.ceil(maxScore * 0.08), Math.ceil(width * 0.003));
  const maxGap = Math.max(1, Math.round(height * 0.012));
  const bands = [];
  let start = -1;
  let lastActive = -1;

  for (let y = 0; y < height; y += 1) {
    const prev = y > 0 ? rowScores[y - 1] : 0;
    const next = y + 1 < height ? rowScores[y + 1] : 0;
    const smooth = (prev + rowScores[y] + next) / 3;
    if (smooth >= activeThreshold) {
      if (start < 0) start = y;
      lastActive = y;
    } else if (start >= 0 && y - lastActive > maxGap) {
      bands.push([start, lastActive]);
      start = -1;
      lastActive = -1;
    }
  }
  if (start >= 0) bands.push([start, lastActive]);

  const merged = [];
  for (const band of bands) {
    if (merged.length === 0 || band[0] - merged[merged.length - 1][1] > maxGap + 1) {
      merged.push(band);
    } else {
  ...
  const minLineHeight = Math.max(4, Math.floor(height * 0.035));
  const lineBands = merged.filter((band) => band[1] - band[0] + 1 >= minLineHeight).slice(0, MAX_LINE_REGIONS);
  if (lineBands.length <= 1) return [imgData];

  return lineBands.map((band) => {
    const pad = Math.max(2, Math.min(8, Math.round((band[1] - band[0] + 1) * 0.25)));
    const y = clamp(band[0] - pad, 0, height - 1);
    const bottom = clamp(band[1] + pad, y, height - 1);
    return cropImageData(imgData, 0, y, width, bottom - y + 1);
  });
}

function buildRowActivityScores(imgData) {
  const width = imgData.width;
  const height = imgData.height;
  const data = imgData.data;
  const stepX = width > 1600 ? 3 : width > 800 ? 2 : 1;
  const threshold = estimateOtsuThreshold(imgData);
  const backgroundIsLight = isLightBackground(imgData, threshold);
  const rowScores = new Uint16Array(height);
  let maxScore = 0;
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += stepX) {
      ...
      const foreground = backgroundIsLight ? gray <= threshold - 4 : gray >= threshold + 4;
      const edge = hasLocalContrast(data, width, height, x, y, gray);
      if (foreground || edge) score += 1;
    }
    rowScores[y] = score;
    if (score > maxScore) maxScore = score;
  }

  return { scores: rowScores, maxScore };
}

function estimateOtsuThreshold(imgData) {
  const width = imgData.width;
  const height = imgData.height;
  const data = imgData.data;
  const hist = new Uint32Array(256);
  const step = width * height > 500000 ? 2 : 1;
  let total = 0;
  ...
  for (let y = 0; y < height; y += step) {
    for (let x = 0; x < width; x += step) {
      const offset = (y * width + x) * 4;
      if (data[offset + 3] < 16) continue;
      hist[Math.round(luminanceAt(data, offset))] += 1;
      total += 1;
    }
  }
...
function isLightBackground(imgData, threshold) {
  const width = imgData.width;
  const height = imgData.height;
  const data = imgData.data;
  const step = width * height > 500000 ? 3 : 2;
  let dark = 0;
  let light = 0;
  ...
  for (let y = 0; y < height; y += step) {
    for (let x = 0; x < width; x += step) {
      const offset = (y * width + x) * 4;
      if (data[offset + 3] < 16) continue;
      if (luminanceAt(data, offset) <= threshold) dark += 1;
      else light += 1;
    }
  }
...
function hasLocalContrast(data, width, height, x, y, gray) {
  const contrastThreshold = 32;
  if (x > 0) {
    const left = (y * width + x - 1) * 4;
    if (Math.abs(gray - luminanceAt(data, left)) >= contrastThreshold) return true;
  }
  if (x + 1 < width) {
    const right = (y * width + x + 1) * 4;
    if (Math.abs(gray - luminanceAt(data, right)) >= contrastThreshold) return true;
  }
  if (y > 0) {
    const up = ((y - 1) * width + x) * 4;
    if (Math.abs(gray - luminanceAt(data, up)) >= contrastThreshold) return true;
  }
  if (y + 1 < height) {
    const down = ((y + 1) * width + x) * 4;
    if (Math.abs(gray - luminanceAt(data, down)) >= contrastThreshold) return true;
  }
  return false;
}
...
function sampleBackgroundColor(imgData) {
  ...
  const height = imgData.height;
  const data = imgData.data;
  const sample = Math.max(2, Math.min(8, Math.floor(Math.min(width, height) / 6)));
  const corners = [
    [0, 0],
  ...
  for (const corner of corners) {
    for (let y = corner[1]; y < Math.min(height, corner[1] + sample); y += 1) {
      for (let x = corner[0]; x < Math.min(width, corner[0] + sample); x += 1) {
        const offset = (y * width + x) * 4;
        if (data[offset + 3] < 16) continue;
        r += data[offset];
        g += data[offset + 1];
...
function colorDistance(data, offset, bg) {
  return Math.abs(data[offset] - bg.r) + Math.abs(data[offset + 1] - bg.g) + Math.abs(data[offset + 2] - bg.b);
}
...
function getRecognitionWidth(rawW, maxW) {
  const bounded = clamp(rawW, REC_MIN_WIDTH, maxW);
  return clamp(Math.ceil(bounded / 32) * 32, REC_MIN_WIDTH, maxW);
}

function preprocess(imgData, maxW, recH) {
  const srcW = imgData.width;
  const srcH = imgData.height;
  const rawW = Math.ceil((srcW / srcH) * recH);
  const recW = getRecognitionWidth(rawW, maxW);
  const resizedW = Math.max(1, Math.min(recW, rawW));
  const srcCanvas = new OffscreenCanvas(srcW, srcH);
  const srcCtx = srcCanvas.getContext("2d");
  srcCtx.putImageData(imgData, 0, 0);

  const dstCanvas = new OffscreenCanvas(recW, recH);
  const dstCtx = dstCanvas.getContext("2d");
  dstCtx.fillStyle = "#808080";
  dstCtx.fillRect(0, 0, recW, recH);
  dstCtx.imageSmoothingEnabled = true;
  dstCtx.imageSmoothingQuality = "high";
  dstCtx.drawImage(srcCanvas, 0, 0, srcW, srcH, 0, 0, resizedW, recH);

  const resized = dstCtx.getImageData(0, 0, recW, recH).data;
  const floatData = new Float32Array(3 * recW * recH);
  const plane = recW * recH;

  for (let y = 0; y < recH; y += 1) {
    for (let x = 0; x < recW; x += 1) {
      let bNorm = 0;
      let gNorm = 0;
      let rNorm = 0;
      if (x < resizedW) {
        const src = (y * recW + x) * 4;
        const r = resized[src] / 255;
        const g = resized[src + 1] / 255;
        const b = resized[src + 2] / 255;
        bNorm = (b - 0.5) / 0.5;
        gNorm = (g - 0.5) / 0.5;
        rNorm = (r - 0.5) / 0.5;
      }
      const offset = y * recW + x;
      floatData[offset] = bNorm;
      floatData[plane + offset] = gNorm;
      floatData[2 * plane + offset] = rNorm;
    }
  }

  return new ortRuntime.Tensor("float32", floatData, [1, 3, recH, recW]);
}
...
function ctcDecode(output) {
  ...
  if (dims.length === 3) {
    timeSteps = dims[1];
    classes = dims[2];
    baseOffset = (time) => time * classes;
  } else if (dims.length === 2) {
    timeSteps = dims[0];
    classes = dims[1];
    baseOffset = (time) => time * classes;
  } else {
    throw new Error("Unsupported OCR output dimensions: [" + dims.join(", ") + "]");
  }

  const decodeChars = getDecodeCharacters(classes);
  let text = "";
  let lastIndex = -1;
  for (let t = 0; t < timeSteps; t += 1) {
    const offset = baseOffset(t);
    let bestIndex = 0;
    ...
    if (bestIndex > 0 && bestIndex !== lastIndex) {
      const char = decodeChars[bestIndex - 1];
      if (char) text += char;
    }
    lastIndex = bestIndex;
  }
...
function getDecodeCharacters(classes) {
  if (classes === chars.length + 1) return chars;
  if (classes === chars.length + 2) return chars.concat(" ");
  if (!warnedClassMismatch) {
    warnedClassMismatch = true;
    console.warn("OCR class count does not match character set", {
      classes,
      characters: chars.length
    });
  }
...
function extractCharactersFromOnnx(buffer) {
  const metadata = readOnnxMetadata(new Uint8Array(buffer));
  const raw = metadata.character || "";
...
async function fetchOptionalText(file) {
  try {
    ...
    console.warn("Optional OCR config not loaded", file, error);
    return "";
  }
}

function extractCharactersFromInferenceYaml(text) {
  const result = [];
  ...
  for (const line of lines) {
    const dictStart = /^(\s*)character_dict\s*:\s*(.*)$/.exec(line);
    if (dictStart) {
      inCharacterDict = true;
      listIndent = null;
      const inline = dictStart[2].trim();
      if (inline.startsWith("[") && inline.endsWith("]")) {
        return inline.slice(1, -1).split(",").map(unquoteYamlScalar).filter((char) => char.length > 0);
      }
      continue;
    }
    ...
    result.push(unquoteYamlScalar(item[2]));
  }

  return result.filter((char) => char.length > 0);
}
...
function readOnnxMetadata(bytes) {
  const decoder = new TextDecoder("utf-8");
  const metadata = {};
  scanProtoMessage(bytes, 0, bytes.length, (field, wire, valueStart, valueEnd) => {
    if (field !== 14 || wire !== 2) return;

    let key = "";
    let value = "";
    scanProtoMessage(bytes, valueStart, valueEnd, (subField, subWire, subStart, subEnd) => {
      if (subWire !== 2) return;
      if (subField === 1) key = decoder.decode(bytes.subarray(subStart, subEnd));
      if (subField === 2) value = decoder.decode(bytes.subarray(subStart, subEnd));
    });
...
function scanProtoMessage(bytes, start, end, visitor) {
  let offset = start;
  while (offset < end) {
    ...
    if (wire === 0) {
      ...
      valueEnd = offset + length.value;
      if (valueEnd > end) throw new Error("ONNX metadata length is out of range");
      visitor(field, wire, offset, valueEnd);
      offset = valueEnd;
      continue;
    } else if (wire === 5) {
      offset += 4;
    ...
    if (offset > end) throw new Error("ONNX metadata field is out of range");
    visitor(field, wire, valueStart, valueEnd);
  }
}
...
function readVarint(bytes, offset, end) {
  ...
  while (offset < end) {
    ...
    shift += 7;
    if (shift > 56) throw new Error("ONNX protobuf varint is too long");
  }
  throw new Error("ONNX protobuf varint is truncated");
}
...
function selectionUI() {
  if (window.__ppocrSelectionActive) return;
  window.__ppocrSelectionActive = true;
  ...
  const outsideClose = (event) => {
    if (!modal) return;
    if (isDraggingModal || Date.now() < suppressOutsideCloseUntil) return;
    if (!modal.contains(event.target)) removeModal();
  };
  ...
  const cleanupSelection = () => {
    window.__ppocrSelectionActive = false;
  ...
  const onMouseUp = async (event) => {
    ...
    box.style.borderColor = "#F5A623";
    box.style.background = "rgba(245, 166, 35, 0.18)";
    showModal("Recognizing...", "Screen OCR");
    ...
    try {
      ...
      cleanupSelection();
      showModal(
        response && response.ok ? response.text : (response && response.error ? response.error : "OCR failed"),
        response && response.ok ? response.modelName : "OCR failed"
      );
    } catch (error) {
      cleanupSelection();
      showModal("Error: " + (error.message || String(error)), "OCR failed");
    }
  };
  ...
  function showModal(text, modelName) {
    removeModal();
    ...
    modal = create("div", {
      ...
      borderRadius: "10px",
      boxShadow: "0 14px 36px rgba(0, 0, 0, 0.28), 0 0 0 1px rgba(255, 255, 255, 0.04)",
      backdropFilter: "blur(7px) saturate(1.06)",
      WebkitBackdropFilter: "blur(7px) saturate(1.06)",
    ...
    const header = create("div", {
      ...
      color: "#cdd6f4",
      borderBottom: "1px solid rgba(205, 214, 244, 0.12)",
      fontSize: "13px",
      fontWeight: "600",
      lineHeight: "1.3",
      cursor: "grab",
    ...
    title.textContent = modelName || "Screen OCR";

    const actions = create("div", { display: "flex", alignItems: "center", gap: "6px" });
    const copy = create("button", buttonStyle());
    copy.type = "button";
    copy.textContent = "Copy";
    const close = create("button", buttonStyle());
    close.type = "button";
    close.textContent = "X";
    close.title = "Close";
    ...
    enableModalDrag(modal, header, actions);
  ...
  function enableModalDrag(target, handle, ignore) {
    let offsetX = 0;
    let offsetY = 0;
    let activePointerId = null;

    const onDragStart = (event) => {
      if (event.button !== 0 || ignore.contains(event.target)) return;
      stop(event);
      ...
      handle.addEventListener("pointermove", onDragMove, true);
      handle.addEventListener("pointerup", onDragEnd, true);
      handle.addEventListener("pointercancel", onDragEnd, true);
    ...
    const onDragMove = (event) => {
      ...
      const left = clampDrag(event.clientX - offsetX, 8, maxLeft);
      const top = clampDrag(event.clientY - offsetY, 8, maxTop);
    ...
    const onDragEnd = (event) => {
      ...
      if (event && event.preventDefault) stop(event);
      ...
      handle.removeEventListener("pointermove", onDragMove, true);
      handle.removeEventListener("pointerup", onDragEnd, true);
      handle.removeEventListener("pointercancel", onDragEnd, true);
    ...
    handle.addEventListener("pointerdown", onDragStart, true);
  }
